...
def classify_error_sync(
    ...
    combined_words = set(f"{error_reason} {error_description}".lower().replace(".", " ").replace(";", " ").replace("_", " ").split())
    if "funds" in combined_words:
        return Diagnosis(
            category=DeclineCategory.INSUFFICIENT_FUNDS,
            recoverability=0.75,
